    ...
    <div className="modal-backdrop" role="presentation">
      <div className="modal" role="dialog" aria-modal="true" aria-labelledby="reason-title">
        ...
        <div className="modal-actions">
          ...
          <button onClick={() => onSubmit(reason)} disabled={reason.trim().length < 2}>确认</button>
        </div>
      </div>
    </div>
  );
...
function MentorReviewsPage() {
  ...
  return (
    <section>
      ...
      <ReasonDialog
        config={dialog ? { title: dialog.title, description: `目标导师：${dialog.mentor.email} (${dialog.mentor.public_id})` } : null}
        onClose={() => setDialog(null)}
      ...
      <State loading={loading} error={error}>
        ...
        <DetailGrid
          items={[
            ...
            ['时区', mentor.timezone],
            ['评分', mentor.rating],
            ['简历', mentor.mentor_resume_url ? <a href={mentor.mentor_resume_url} target="_blank" rel="noreferrer">打开简历</a> : '-'],
