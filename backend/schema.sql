...
-- Per-user thread archive visibility. Archiving hides the thread from the list until
-- a newer message arrives; older messages remain stored and can reappear later.
CREATE TABLE IF NOT EXISTS `message_thread_archives` (
  `thread_id` BIGINT NOT NULL,
  `user_id` INT NOT NULL,
  `archived_after_message_id` BIGINT NULL,
  `archived_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`thread_id`, `user_id`),
  KEY `idx_message_thread_archives_user` (`user_id`, `archived_at`),
  CONSTRAINT `fk_message_thread_archives_thread` FOREIGN KEY (`thread_id`) REFERENCES `message_threads`(`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_message_thread_archives_user` FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
-- Per-user soft delete visibility for message items ("delete for me").
CREATE TABLE IF NOT EXISTS `message_item_hidden_for_users` (
  `message_item_id` BIGINT NOT NULL,
  `user_id` INT NOT NULL,
  `hidden_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`message_item_id`, `user_id`),
  KEY `idx_message_item_hidden_user` (`user_id`),
  CONSTRAINT `fk_message_item_hidden_message` FOREIGN KEY (`message_item_id`) REFERENCES `message_items`(`id`) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS `course_session_disputes` (
  ...
  `mentor_result_email_sent_at` TIMESTAMP NULL DEFAULT NULL,
...
-- 17b) Mentor cooperation agreements: contract-bound email verification,
-- immutable final artifacts, and signing/download audit evidence.
