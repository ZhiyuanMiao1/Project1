...
const parseWindowText = (windowText: unknown, createdAt: Date): ParsedWindow | null => {
  ...
  if (year == null) {
    ...
    type Candidate = { year: number; startUtc: Date; diffMs: number };
    const computed: Candidate[] = candidates.map((candidateYear) => {
...
async function main() {
  ...
  const [acceptedRows] = await connection.query<any[]>(
    `
    SELECT
  ...
  for (const row of acceptedRows || []) {
    ...
    if (!Number.isFinite(studentUserId) || studentUserId <= 0) continue;
