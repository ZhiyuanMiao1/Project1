...
const baseMentor = (overrides: Partial<RecommendationInput> = {}): RecommendationInput => ({
  userId: 1,
  id: 'm1',
  name: 'Mentor',
  displayName: 'Mentor',
...
const ranked = rankMentorsForRecommendation([
  baseMentor({
    id: 'm-inactive',
...
assert.equal(ranked.length, 3, 'only filters mentors with no profile signal');
assert.ok(ranked.some((mentor) => mentor.id === 'm-inactive'), 'keeps inactive mentors for score downranking');
assert.ok(ranked.some((mentor) => mentor.id === 'm-low-profile'), 'keeps low-profile mentors for score downranking');
const newMentor = ranked.find((mentor) => mentor.id === 'm-new-no-history');
assert.ok(newMentor, 'keeps qualified new mentor');
assert.equal(newMentor?.recommendation.performanceScore, 0.55, 'new mentor gets neutral performance score');

const clustered = rankMentorsForRecommendation([
...
assert.notDeepEqual(
  clustered.slice(0, 3).map((mentor) => mentor.primaryCourseKey),
  ['ai', 'ai', 'ai'],
  'light diversification avoids three identical primary courses at the top'
);
