...
async function ensureColumn(name: string, sql: string) {
...
async function main() {
  ...
  const [anomalyRows] = await pool.query<any[]>(
    `SELECT u.id, u.lesson_balance_hours, COALESCE(SUM(bo.topup_hours), 0) AS purchased_hours
     FROM users u
  ...
  try {
    ...
    await conn.query(
      `INSERT IGNORE INTO billing_hour_allocations
    ...
    const [validationRows] = await conn.query<any[]>(
      `SELECT u.id,
              u.lesson_balance_hours,
    ...
    const [summaryRows] = await pool.query<any[]>(
      `SELECT COUNT(*) AS credited_orders,
    ...
    console.log(
      `退款迁移完成：${Number(summary.credited_orders || 0)} 笔已充值订单，`
