...
let cachedCnyToUsdRate: CachedRate | null = null;
let rateRefreshPromise: Promise<CachedRate> | null = null;
...
const getFrankfurterApiBase = (): string => {
...
const getCacheDurationMs = (): number =>
  parseBoundedNumber(process.env.FX_CACHE_MINUTES, DEFAULT_CACHE_MINUTES, 1, 24 * 60) * 60 * 1000;

const getMaxStaleMs = (): number =>
  parseBoundedNumber(process.env.FX_MAX_STALE_HOURS, DEFAULT_MAX_STALE_HOURS, 1, 7 * 24) * 60 * 60 * 1000;

const getQuoteDurationMs = (): number =>
  parseBoundedNumber(process.env.FX_QUOTE_MINUTES, DEFAULT_QUOTE_MINUTES, 2, 24 * 60) * 60 * 1000;

const getMarkupBps = (): number =>
  parseBoundedNumber(process.env.FX_MARKUP_BPS, 0, 0, 5000);

const getRequestTimeoutMs = (): number =>
  parseBoundedNumber(process.env.FX_REQUEST_TIMEOUT_MS, DEFAULT_TIMEOUT_MS, 1000, 30000);

const fetchLatestCnyToUsdRate = async (): Promise<CachedRate> => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Frankfurter rate request failed (${response.status}): ${String(data?.message || response.statusText)}`
      );
    }
...
const getCnyToUsdRate = async (): Promise<CachedRate & { stale: boolean }> => {
  const now = Date.now();
...
const roundUpUsdCents = (value: number): number =>
  Number((Math.ceil(value * 100 - 1e-9) / 100).toFixed(2));

export async function quoteCnyToUsd(amountCny: number): Promise<FxQuote> {
  ...
  return {
    quoteId: `frankfurter:${rateSnapshot.rateDate}:${randomUUID()}`,
