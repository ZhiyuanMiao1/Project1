...
const sanitizeStringArray = (value: unknown, maxItems: number, maxItemLen: number) => {
...
const sanitizeAttachments = (raw: unknown, requestId: number) => {
  ...
  for (const item of raw) {
    ...
    if (!/^[0-9a-fA-F]{32}$/.test(fileId)) continue;
    ...
    if (!sizeBytes || sizeBytes < 1 || sizeBytes > 1024 * 1024 * 200) continue; // <= 200MB hard cap
    const ossKey = typeof input.ossKey === 'string' ? input.ossKey.trim() : '';
    if (!ossKey || ossKey.length > 1024) continue;
    if (!ossKey.startsWith(prefix)) continue;
    const fileUrl = typeof input.fileUrl === 'string' ? input.fileUrl.trim() : '';
    if (!fileUrl || fileUrl.length > 2048) continue;
...
const buildUpdate = (body: any): RequestUpdate => {
  ...
  if (hasOwn(body, 'courseFocus')) update.courseFocus = sanitizeString(body.courseFocus, 8000);
  if (hasOwn(body, 'format')) update.format = sanitizeString(body.format, 64);
  if (hasOwn(body, 'milestone')) update.milestone = sanitizeString(body.milestone, 8000);
  ...
  if (hasOwn(body, 'timeZone')) update.timeZone = sanitizeString(body.timeZone, 64);
  ...
  if (hasOwn(body, 'contactName')) update.contactName = sanitizeString(body.contactName, 100);
  if (hasOwn(body, 'contactMethod')) update.contactMethod = sanitizeString(body.contactMethod, 32);
  if (hasOwn(body, 'contactValue')) update.contactValue = sanitizeString(body.contactValue, 200);

  return update;
};

const applyUpdate = async (conn: any, requestId: number, userId: number, update: RequestUpdate) => {
  const sets: string[] = [];
  const args: any[] = [];
  ...
  if (hasOwn(update, 'courseTypesJson')) add('course_types_json', update.courseTypesJson);
  if (hasOwn(update, 'courseFocus')) add('course_focus', update.courseFocus);
  if (hasOwn(update, 'format')) add('format', update.format);
  if (hasOwn(update, 'milestone')) add('milestone', update.milestone);
  if (hasOwn(update, 'totalCourseHours')) add('total_course_hours', update.totalCourseHours);
  if (hasOwn(update, 'timeZone')) add('time_zone', update.timeZone);
  if (hasOwn(update, 'sessionDurationHours')) add('session_duration_hours', update.sessionDurationHours);
  if (hasOwn(update, 'scheduleJson')) add('schedule_json', update.scheduleJson);
  if (hasOwn(update, 'contactName')) add('contact_name', update.contactName);
  if (hasOwn(update, 'contactMethod')) add('contact_method', update.contactMethod);
  if (hasOwn(update, 'contactValue')) add('contact_value', update.contactValue);
...
router.get('/draft', requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    const [rows] = await pool.execute<any[]>(
      "SELECT * FROM course_requests WHERE user_id = ? AND status = 'draft' ORDER BY updated_at DESC LIMIT 1",
      [req.user!.id]
    );
    const draft = rows?.[0];
...
router.delete('/drafts/:id', requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [result] = await pool.execute<any>(
      "DELETE FROM course_requests WHERE id = ? AND user_id = ? AND status IN ('draft', 'submitted')",
      [requestId, req.user!.id]
    );
...
router.post(
  ...
  [
    ...
    body('courseTypes').optional().isArray(),
    body('courseFocus').optional().isString().isLength({ max: 8000 }),
    body('format').optional().isString().isLength({ max: 64 }),
    body('milestone').optional().isString().isLength({ max: 8000 }),
    body('totalCourseHours').optional(),
    body('timeZone').optional().isString().isLength({ max: 64 }),
    body('sessionDurationHours').optional(),
    body('daySelections').optional().custom((v) => v && typeof v === 'object' && !Array.isArray(v)),
    body('contactName').optional().isString().isLength({ max: 100 }),
    body('contactMethod').optional().isString().isLength({ max: 32 }),
    body('contactValue').optional().isString().isLength({ max: 200 }),
  ...
  async (req: Request, res: Response) => {
    ...
    const requestedId = parseFiniteNumber((req.body as any)?.requestId);
    ...
    try {
      ...
      if (hasAttachments) {
        const attachments = sanitizeAttachments((req.body as any).attachments, requestId) || [];
        await conn.execute('DELETE FROM course_request_attachments WHERE request_id = ?', [requestId]);
        for (const att of attachments) {
          await conn.execute(
            `INSERT INTO course_request_attachments
              (request_id, file_id, original_file_name, ext, content_type, size_bytes, oss_key, file_url)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [requestId, att.fileId, att.fileName, att.ext, att.contentType, att.sizeBytes, att.ossKey, att.fileUrl]
          );
        }
      }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      await conn.execute(
        "UPDATE course_requests SET status = 'submitted', submitted_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?",
        [requestId, userId]
      );
