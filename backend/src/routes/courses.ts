...
const ensureCourseDisputeSchema = async () => {
  if (courseDisputeSchemaEnsured) return true;
  ...
  const ensureDisputeColumn = async (sql: string) => {
    try { await query(sql); } catch (e: any) {
      if (String(e?.code || '') !== 'ER_DUP_FIELDNAME' && !String(e?.message || '').includes('Duplicate column name')) throw e;
    }
  };
...
router.post('/:courseId/disputes', requireAuth, async (req: Request, res: Response) => {
  if (!req.user) return res.status(401).json({ error: 'unauthorized' });
  ...
  if (
    description.length < COURSE_DISPUTE_DESCRIPTION_MIN_LENGTH
  ...
  try {
    ...
    const publicId = `CD${Date.now().toString(36)}${crypto.randomBytes(4).toString('hex')}`.toUpperCase();
    await connection.query(
      `
