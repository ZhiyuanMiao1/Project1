...
const ensurePreferredLanguageColumn = async () => {
  if (preferredLanguageColumnEnsured) return true;
  try {
    await dbQuery("ALTER TABLE account_settings ADD COLUMN preferred_language ENUM('zh-CN','en') NOT NULL DEFAULT 'zh-CN'");
...
const ensureMentorResumeColumn = async () => {
  ...
  try {
    await dbQuery('ALTER TABLE account_settings MODIFY COLUMN mentor_resume_url TEXT NULL');
    mentorResumeColumnEnsured = true;
    return true;
  } catch (e: any) {
    if (isMissingMentorResumeColumnError(e)) {
      try {
        await dbQuery('ALTER TABLE account_settings ADD COLUMN mentor_resume_url TEXT NULL');
        mentorResumeColumnEnsured = true;
        return true;
      } catch (inner: any) {
...
router.post(
  ...
  requireAuth,
  [
    body('resumeUrl').optional().isString().trim().isLength({ min: 1, max: 500 }).withMessage('请先上传简历'),
...
router.get('/reviews-summary', requireAuth, async (req: Request, res: Response) => {
  if (!req.user) return res.status(401).json({ error: '未授权' });

  try {
    const [studentCountRows, mentorCountRows, studentReviewRows, mentorReviewRows] = await Promise.all([
      dbQuery<any[]>(
        'SELECT COUNT(*) AS total_count FROM course_session_reviews WHERE student_user_id = ?',
        [req.user.id]
      ),
      dbQuery<any[]>(
        'SELECT COUNT(*) AS total_count FROM course_session_reviews WHERE mentor_user_id = ?',
        [req.user.id]
      ),
      dbQuery<ReviewSummaryRow[]>(
        `
          SELECT
            csr.id,
...
router.put(
  '/language',
  requireAuth,
  [body('language').isIn(['zh-CN', 'en']).withMessage('语言设置无效')],
  async (req: Request, res: Response) => {
    if (!req.user) return res.status(401).json({ error: '未授权' });
    const errors = validationResult(req);
    if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
    ...
    try {
      ...
      await dbQuery(
        `INSERT INTO account_settings (user_id, preferred_language)
         VALUES (?, ?)
         ON DUPLICATE KEY UPDATE preferred_language = VALUES(preferred_language), updated_at = CURRENT_TIMESTAMP`,
