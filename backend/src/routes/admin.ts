...
router.patch('/mentor-payroll/:mentorUserId/settlement-method', requireAdminAuth, async (req: Request, res: Response) => {
  const mentorUserId = toPositiveInt(req.params.mentorUserId, 0);
  const settlementMethod = safeString((req.body as any)?.settlementMethod, 20).toLowerCase();
  if (!mentorUserId) return res.status(400).json({ error: '导师 ID 无效' });
  ...
  try {
    ...
    if (!mentors?.length) return res.status(404).json({ error: '未找到已通过审核的导师' });
    const beforeRows = await query<any[]>(
      'SELECT settlement_method FROM mentor_payroll_profiles WHERE mentor_user_id = ? LIMIT 1',
      [mentorUserId]
    );
    await query(
      `INSERT INTO mentor_payroll_profiles (mentor_user_id, settlement_method, updated_by_admin_id)
       VALUES (?, ?, ?)
       ON DUPLICATE KEY UPDATE settlement_method = VALUES(settlement_method),
         updated_by_admin_id = VALUES(updated_by_admin_id)`,
      [mentorUserId, settlementMethod, req.admin?.adminId || null]
    );
      ...
      query<any[]>(
        `SELECT COUNT(*) AS count
         ...
         WHERE ur.role = 'mentor'
           ...
           AND COALESCE(pay.status, 'pending') <> 'paid'`,
        [defaultHourlyRate]
      ),
      query<any[]>(
