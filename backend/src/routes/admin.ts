...
router.post('/mentors/:userId/approve', requireAdminAuth, async (req: Request, res: Response) => {
  ...
  const reason = readReason(req);
  if (!userId) return res.status(400).json({ error: '无效导师ID' });
  if (!reason) return res.status(400).json({ error: '请填写审核原因' });
...
router.get('/appointments', requireAdminAuth, async (req: Request, res: Response) => {
  const { page, limit, offset } = getPaging(req);
  const q = safeString(req.query.q, 100);
...
router.patch('/appointments/:appointmentId/status', requireAdminAuth, async (req: Request, res: Response) => {
  const appointmentId = toPositiveInt(req.params.appointmentId, 0);
  const status = safeString((req.body as any)?.status, 30);
  const reason = readReason(req);
  if (!appointmentId) return res.status(400).json({ error: '无效预约ID' });
  if (!APPOINTMENT_STATUSES.has(status)) return res.status(400).json({ error: '无效预约状态' });
  if (!reason) return res.status(400).json({ error: '请填写操作原因' });
  ...
  try {
    ...
    await conn.execute(
      `INSERT INTO appointment_statuses (appointment_message_id, status, updated_by_user_id)
       VALUES (?, ?, ?)
       ON DUPLICATE KEY UPDATE status = VALUES(status), updated_by_user_id = VALUES(updated_by_user_id), updated_at = CURRENT_TIMESTAMP`,
      [appointmentId, status, updatedByUserId]
    );
    await conn.commit();
    const afterRows = await query<any[]>(
      "SELECT appointment_message_id AS id, status, updated_at FROM appointment_statuses WHERE appointment_message_id = ? LIMIT 1",
      [appointmentId]
    );
    const after = afterRows?.[0];
