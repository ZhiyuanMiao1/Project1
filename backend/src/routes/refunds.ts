...
import {
  computeRefundPricing,
  parseRefundHours,
  type RefundPricingQuote,
} from '../services/refundPricing';
import {
  fetchJson,
...
const normalizeRefundStatus = (value: unknown) => {
...
type RefundOrderRow = {
  ...
  provider: string;
  topup_hours: number | string;
  remaining_hours: number | string | null;
  unit_price_cny: number | string;
  pricing_version: string;
  standard_unit_price_cny: number | string;
  discount_threshold_hours: number | string;
  discount_unit_price_cny: number | string;
...
const normalizeProvider = (value: unknown) => String(value || '').trim().toLowerCase();

const getOriginalPaidAmount = (order: RefundOrderRow) => {
  const currency = String(order.currency_code || '').trim().toUpperCase();
  if (currency === 'CNY') return toNumber(order.amount_cny);
  const providerAmount = toNumber(order.amount_usd);
  return providerAmount > 0 ? providerAmount : toNumber(order.amount_cny);
};

const canRefundOrder = (order: RefundOrderRow | null): order is RefundOrderRow => {
  if (!order) return false;
  const provider = normalizeProvider(order.provider);
  if (!REFUNDABLE_PROVIDERS.has(provider)) return false;
  return provider !== 'paypal' || Boolean(order.paypal_capture_id);
};

const aggregateActiveRefunds = (rows: any[] = []): RefundAggregate => {
  return rows.reduce<RefundAggregate>((acc, row) => {
    const status = normalizeRefundStatus(row?.status);
    if (!ACTIVE_REFUND_STATUSES.has(status)) return acc;
...
const calculateOrderQuote = (
  order: RefundOrderRow,
  active: RefundAggregate,
  requestedHours: number
): RefundPricingQuote => {
  return computeRefundPricing({
...
async function loadOrderAndRefunds(
  conn: PoolConnection,
  userId: number,
  orderId: number,
  lock = false
  ...
  const [rawOrderRows] = await conn.query<any[]>(
    `SELECT id, user_id, provider, topup_hours, remaining_hours, unit_price_cny,
  ...
  const orderRows = rawOrderRows as RefundOrderRow[];
  const order = orderRows?.[0] || null;
  if (!order) return { order: null, refunds: [], active: aggregateActiveRefunds() };

  const [refundRows] = await conn.query<any[]>(
    `SELECT id, requested_hours, amount_cny, amount_original, status
     FROM billing_refunds
     WHERE billing_order_id = ?${lock ? ' FOR UPDATE' : ''}`,
    [orderId]
  );
  return { order, refunds: refundRows || [], active: aggregateActiveRefunds(refundRows || []) };
}

async function getUpcomingScheduledHours(userId: number) {
...
async function getWalletSummary(userId: number) {
  const [balanceRows, grossRows, refundRows] = await Promise.all([
    dbQuery<any[]>('SELECT lesson_balance_hours FROM users WHERE id = ? LIMIT 1', [userId]),
    dbQuery<any[]>(
      `SELECT
       ...
       FROM billing_orders
       WHERE user_id = ? AND credited_at IS NOT NULL`,
      [userId]
    ),
    dbQuery<any[]>(
...
async function fetchRefundRow(refundId: number) {
  const rows = await dbQuery<any[]>(
    `SELECT br.*, bo.paypal_capture_id
...
async function releaseFailedRefund(refundId: number, failureCode: string, failureMessage: string, payload: any) {
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
    const [seedRows] = await conn.query<any[]>(
      'SELECT user_id FROM billing_refunds WHERE id = ? LIMIT 1',
      [refundId]
    );
    ...
    await conn.query('SELECT id FROM users WHERE id = ? LIMIT 1 FOR UPDATE', [userId]);
    const [refundRows] = await conn.query<any[]>(
      `SELECT id, billing_order_id, requested_hours, status, balance_reserved
       FROM billing_refunds
       WHERE id = ?
       LIMIT 1
       FOR UPDATE`,
    ...
    if (Number(refund.balance_reserved) === 1) {
      ...
      await conn.query(
        ...
        [hours, Number(refund.billing_order_id)]
      );
      await conn.query(
    ...
    await conn.query(
      `UPDATE billing_refunds
       ...
       WHERE id = ?`,
      [failureCode.slice(0, 80), failureMessage.slice(0, 500), JSON.stringify(payload || {}), refundId]
    );
    await conn.commit();
    return fetchRefundRow(refundId);
  } catch (error) {
...
async function saveProviderState(refundId: number, data: any) {
  ...
  await dbQuery(
    `UPDATE billing_refunds
     SET status = ?,
         ...
         updated_at = CURRENT_TIMESTAMP
     WHERE id = ?`,
    [mappedStatus, refundProviderId, JSON.stringify(data || {}), mappedStatus, refundId]
  );
  return fetchRefundRow(refundId);
}

async function processPayPalRefund(refundId: number) {
  const refund = await fetchRefundRow(refundId);
  if (!refund) return null;
  const currentStatus = normalizeRefundStatus(refund.status);
  if (FINAL_REFUND_STATUSES.has(currentStatus)) return refund;

  const runtime = getPayPalRuntimeConfig();
  if (!runtime) {
    return releaseFailedRefund(
      refundId,
      'PAYPAL_NOT_CONFIGURED',
      'PayPal 退款服务暂不可用，请稍后重试',
      {}
    );
  }
  ...
  try {
    ...
    if (!captureId) {
      return releaseFailedRefund(
        refundId,
        'CAPTURE_ID_MISSING',
        '原支付缺少 PayPal Capture ID，无法自动退款',
        {}
      );
    }

    const result = providerRefundId
      ? await fetchJson(`${runtime.apiBase}/v2/payments/refunds/${encodeURIComponent(providerRefundId)}`, {
          method: 'GET',
          headers: { Authorization: `Bearer ${token.accessToken}` },
        })
      : await fetchJson(`${runtime.apiBase}/v2/payments/captures/${encodeURIComponent(captureId)}/refund`, {
          method: 'POST',
          headers: {
      ...
      await dbQuery(
        `UPDATE billing_refunds
         SET status = 'PROCESSING',
             ...
             updated_at = CURRENT_TIMESTAMP
         WHERE id = ?`,
        [issue.slice(0, 80), message.slice(0, 500), JSON.stringify(result.data || {}), refundId]
      );
      return fetchRefundRow(refundId);
    }
...
router.get('/eligible-orders', requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    const [orderRows, refundRows, upcomingScheduledHours, wallet] = await Promise.all([
      dbQuery<any[]>(
        `SELECT bo.id, bo.user_id, bo.provider, bo.topup_hours, bo.remaining_hours,
    ...
    const refundRowsByOrder = new Map<string, any[]>();
    ...
    const orders = (orderRows || []).map((order) => {
      ...
      return {
        ...
        paidAmount: {
          ...
          value: Number(getOriginalPaidAmount(order as RefundOrderRow).toFixed(2)),
        },
        maximumRefund: {
...
router.post('/quote', requireAuth, async (req: Request, res: Response) => {
  if (!req.user) return res.status(401).json({ error: '未授权' });
  const orderId = Number.parseInt(String((req.body as any)?.orderId || ''), 10);
  const hours = parseRefundHours((req.body as any)?.hours);
  if (!Number.isFinite(orderId) || orderId <= 0) {
    return res.status(400).json({ error: '无效充值订单' });
  }
  if (!hours) return res.status(400).json({ error: '退款课时需为 0.25 小时的倍数' });
...
router.post('/', requireAuth, async (req: Request, res: Response) => {
  ...
  const orderId = Number.parseInt(String((req.body as any)?.orderId || ''), 10);
  const hours = parseRefundHours((req.body as any)?.hours);
  const clientRequestId = String((req.body as any)?.clientRequestId || '').trim();
  const expectedAmount = toNumber((req.body as any)?.expectedAmount, -1);
  if (!Number.isFinite(orderId) || orderId <= 0) {
    return res.status(400).json({ error: '无效充值订单' });
  }
  if (!hours) return res.status(400).json({ error: '退款课时需为 0.25 小时的倍数' });
  ...
  try {
    ...
    const [existingRows] = await conn.query<any[]>(
      'SELECT id, user_id FROM billing_refunds WHERE public_id = ? LIMIT 1 FOR UPDATE',
      [clientRequestId]
    );
...
router.get('/:refundId/status', requireAuth, async (req: Request, res: Response) => {
  if (!req.user) return res.status(401).json({ error: '未授权' });
  const publicId = String(req.params.refundId || '').trim();
  if (!publicId) return res.status(400).json({ error: '无效退款记录' });
  try {
    const rows = await dbQuery<any[]>(
      'SELECT id FROM billing_refunds WHERE public_id = ? AND user_id = ? LIMIT 1',
