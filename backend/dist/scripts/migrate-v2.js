...
async function main() {
    ...
    const tableExists = async (tableName) => {
        const row = await queryScalar('SELECT COUNT(*) AS c FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ?', [tableName]);
        return Number(row?.c || 0) > 0;
    };
    const columnExists = async (tableName, columnName) => {
        const row = await queryScalar('SELECT COUNT(*) AS c FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND COLUMN_NAME = ?', [tableName, columnName]);
        return Number(row?.c || 0) > 0;
    };
    const listForeignKeys = async (tableName) => {
        const [rows] = await connection.query('SELECT CONSTRAINT_NAME FROM information_schema.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_SCHEMA = DATABASE() AND TABLE_NAME = ?', [tableName]);
        return (rows || []).map((r) => String(r.CONSTRAINT_NAME)).filter(Boolean);
    };
    const dropAllForeignKeys = async (tableName) => {
  ...
  `);
    await connection.query("INSERT IGNORE INTO `role_counters` (`role`, `next_serial`) VALUES ('student', 0), ('mentor', 0)");
    ...
    if (await tableExists('mentor_profiles_v1')) {
       ...
       JOIN users_v1 u ON u.id = mp.user_id`);
        ...
        if (emails.length) {
            ...
            const [uRows] = await connection.query(`SELECT id, email FROM users WHERE email IN (${placeholders})`, emails);
        ...
        for (const [email, list] of byEmail.entries()) {
            ...
            await connection.query(`INSERT INTO mentor_profiles (
