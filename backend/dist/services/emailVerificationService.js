...
exports.consumeEmailVerificationToken = exports.verifyEmailCode = exports.sendEmailVerificationCode = exports.ensureEmailVerificationTable = exports.EmailVerificationError = void 0;
...
const hashEmailCode = ({ email, purpose, code, salt, }) => crypto_1.default
    .createHmac('sha256', EMAIL_CODE_SECRET)
    .update(`${salt}:${normalizeEmail(email)}:${purpose}:${code}`)
    .digest('hex');
const hashVerificationToken = (token) => crypto_1.default.createHash('sha256').update(String(token || '')).digest('hex');
const generateNumericCode = (length) => {
    const size = Math.max(4, Math.min(8, length));
    const max = 10 ** size;
    return String(crypto_1.default.randomInt(0, max)).padStart(size, '0');
};
...
const ensureEmailVerificationTable = async () => {
    if (emailVerificationTableEnsured)
        return true;
    await (0, db_1.query)(`CREATE TABLE IF NOT EXISTS \`email_verification_codes\` (
...
exports.ensureEmailVerificationTable = ensureEmailVerificationTable;
const assertSupportedPurpose = (purpose) => {
...
const sendEmailVerificationCode = async ({ email, purpose, ip, userAgent, }) => {
    await (0, exports.ensureEmailVerificationTable)();
    const normalizedEmail = normalizeEmail(email);
    const normalizedPurpose = assertSupportedPurpose(purpose);
    const now = new Date();
    const latestRows = await (0, db_1.query)(`SELECT id, email, purpose, code_hash, code_salt, attempt_count, max_attempts, resend_available_at, expires_at,
            verified_at, consumed_at, invalidated_at, verification_token_hash, verification_expires_at, verification_consumed_at
     FROM email_verification_codes
     WHERE email = ? AND purpose = ?
     ORDER BY id DESC
     LIMIT 1`, [normalizedEmail, normalizedPurpose]);
    ...
    if (isLatestStillPending && resendAt && resendAt.getTime() > now.getTime()) {
        ...
        throw new EmailVerificationError('EMAIL_CODE_RATE_LIMIT', `验证码发送过于频繁，请 ${retryAfterSeconds} 秒后再试`, 429, { retryAfterSeconds });
    }
    ...
    await (0, db_1.query)(`UPDATE email_verification_codes
     SET invalidated_at = COALESCE(invalidated_at, ?)
     WHERE email = ? AND purpose = ? AND invalidated_at IS NULL AND verification_consumed_at IS NULL`, [now, normalizedEmail, normalizedPurpose]);
    const insertResult = await (0, db_1.query)(`INSERT INTO email_verification_codes
    ...
    const recordId = Number(insertResult?.insertId || 0);
    try {
        ...
        if (recordId) {
            await (0, db_1.query)('UPDATE email_verification_codes SET invalidated_at = COALESCE(invalidated_at, ?) WHERE id = ?', [new Date(), recordId]);
        }
...
const verifyEmailCode = async ({ email, purpose, code, }) => {
    ...
    try {
        ...
        const [rows] = await conn.execute(`SELECT id, email, purpose, code_hash, code_salt, attempt_count, max_attempts, resend_available_at, expires_at,
              verified_at, consumed_at, invalidated_at, verification_token_hash, verification_expires_at, verification_consumed_at
       FROM email_verification_codes
       WHERE email = ? AND purpose = ?
       ORDER BY id DESC
       LIMIT 1
       FOR UPDATE`, [normalizedEmail, normalizedPurpose]);
        ...
        if (!expiresAt || expiresAt.getTime() <= now.getTime()) {
            await conn.execute('UPDATE email_verification_codes SET invalidated_at = COALESCE(invalidated_at, ?) WHERE id = ?', [now, row.id]);
            await conn.commit();
        ...
        if (expectedHash !== row.code_hash) {
            ...
            await conn.execute(`UPDATE email_verification_codes
         SET attempt_count = ?, last_attempt_at = ?, invalidated_at = COALESCE(invalidated_at, ?)
         WHERE id = ?`, [nextAttempts, now, lockNow, row.id]);
            await conn.commit();
            throw new EmailVerificationError(attemptsRemaining > 0 ? 'EMAIL_CODE_INVALID' : 'EMAIL_CODE_TOO_MANY_ATTEMPTS', attemptsRemaining > 0 ? '验证码错误，请重新输入' : '验证码错误次数过多，请重新发送', attemptsRemaining > 0 ? 400 : 429, { attemptsRemaining });
        }
        const verificationToken = crypto_1.default.randomBytes(24).toString('base64url');
       ...
       WHERE id = ?`, [now, now, now, verificationTokenHash, verificationExpiresAt, now, row.id]);
        await conn.commit();
        return {
            message: '邮箱验证通过',
            verificationToken,
...
const consumeEmailVerificationToken = async ({ email, purpose, verificationToken, }) => {
    await (0, exports.ensureEmailVerificationTable)();
    const normalizedEmail = normalizeEmail(email);
    const normalizedPurpose = assertSupportedPurpose(purpose);
    ...
    try {
        await conn.beginTransaction();
        const [rows] = await conn.execute(`SELECT id, verification_expires_at, verification_consumed_at
       FROM email_verification_codes
       WHERE email = ? AND purpose = ? AND verification_token_hash = ?
       ORDER BY id DESC
       LIMIT 1
       FOR UPDATE`, [normalizedEmail, normalizedPurpose, tokenHash]);
        const row = Array.isArray(rows) ? rows[0] : undefined;
        if (!row) {
        ...
        if (!verificationExpiresAt || verificationExpiresAt.getTime() <= now.getTime()) {
            await conn.execute('UPDATE email_verification_codes SET verification_consumed_at = COALESCE(verification_consumed_at, ?), invalidated_at = COALESCE(invalidated_at, ?) WHERE id = ?', [now, now, row.id]);
            await conn.commit();
            throw new EmailVerificationError('EMAIL_VERIFICATION_EXPIRED', '邮箱验证已过期，请重新获取验证码', 401);
        }
        await conn.execute('UPDATE email_verification_codes SET verification_consumed_at = ? WHERE id = ? AND verification_consumed_at IS NULL', [now, row.id]);
        await conn.commit();
        return { id: row.id };
    }
    catch (error) {
