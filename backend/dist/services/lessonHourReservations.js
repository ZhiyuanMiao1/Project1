...
exports.ensureLessonHourReservationSchema = void 0;
exports.releaseLessonHoursReservation = releaseLessonHoursReservation;
exports.reserveLessonHours = reserveLessonHours;
exports.settleLessonHours = settleLessonHours;
...
const ensureLessonHourReservationSchema = async () => {
...
exports.ensureLessonHourReservationSchema = ensureLessonHourReservationSchema;
const restoreSourceHours = async (conn, sourceType, sourceId, hours) => {
    if (hours <= EPSILON)
        return;
    if (sourceType === 'grant') {
        await conn.query('UPDATE platform_lesson_hour_grants SET remaining_hours = remaining_hours + ? WHERE id = ?', [hours, sourceId]);
        return;
    }
    await conn.query('UPDATE billing_orders SET remaining_hours = remaining_hours + ? WHERE id = ?', [hours, sourceId]);
};
async function releaseLessonHoursReservation(conn, courseSessionId) {
    const [reservationRows] = await conn.query(`SELECT id, student_user_id, reserved_hours, status
     FROM course_session_hour_reservations
     WHERE course_session_id = ?
     LIMIT 1
     FOR UPDATE`, [courseSessionId]);
    const reservation = reservationRows?.[0];
    if (!reservation || String(reservation.status) !== 'active')
        return 0;
    const [sourceRows] = await conn.query(`SELECT source_type, source_id, hours
     FROM course_session_hour_reservation_sources
     WHERE reservation_id = ?
     ORDER BY id ASC
     FOR UPDATE`, [Number(reservation.id)]);
    const reservedHours = toHours(reservation.reserved_hours);
    const sourceHoursTotal = Number((sourceRows || []).reduce((sum, source) => sum + toHours(source.hours), 0).toFixed(2));
    if (Math.abs(sourceHoursTotal - reservedHours) > EPSILON) {
        throw new walletHours_1.WalletHoursError('HOUR_ALLOCATION_MISMATCH', '冻结课时明细不完整，请联系平台处理');
    }
    for (const source of sourceRows || []) {
        await restoreSourceHours(conn, String(source.source_type) === 'grant' ? 'grant' : 'order', Number(source.source_id), toHours(source.hours));
    }
    await conn.query('UPDATE users SET lesson_balance_hours = lesson_balance_hours + ? WHERE id = ?', [reservedHours, Number(reservation.student_user_id)]);
    await conn.query(`UPDATE course_session_hour_reservations
     SET status = 'released', released_at = CURRENT_TIMESTAMP
     WHERE id = ?`, [Number(reservation.id)]);
    return reservedHours;
}
async function reserveLessonHours(conn, studentUserId, courseSessionId, rawHours) {
    const hours = toHours(rawHours);
    if (hours <= 0) {
        throw new walletHours_1.WalletHoursError('HOUR_ALLOCATION_MISMATCH', '无效课时');
    }
    const [existingRows] = await conn.query(`SELECT id, reserved_hours, status
     FROM course_session_hour_reservations
     WHERE course_session_id = ?
     LIMIT 1
     FOR UPDATE`, [courseSessionId]);
    ...
    const [userRows] = await conn.query('SELECT lesson_balance_hours FROM users WHERE id = ? LIMIT 1 FOR UPDATE', [studentUserId]);
    const balance = toHours(userRows?.[0]?.lesson_balance_hours);
    if (balance + EPSILON < hours) {
        throw new walletHours_1.WalletHoursError('INSUFFICIENT_HOURS', '剩余课时不足，请先充值后再接受预约');
    }
    ...
    if (unallocated > EPSILON) {
        throw new walletHours_1.WalletHoursError('REFUND_SCHEMA_REQUIRED', '钱包课时明细尚未完成升级，请联系平台处理');
    }
    ...
    if (reservationId > 0) {
       ...
       WHERE id = ?`, [studentUserId, hours, reservationId]);
        await conn.query('DELETE FROM course_session_hour_reservation_sources WHERE reservation_id = ?', [reservationId]);
    }
    else {
        const [insertResult] = await conn.query(`INSERT INTO course_session_hour_reservations
         (course_session_id, student_user_id, reserved_hours, status)
       VALUES (?, ?, ?, 'active')`, [courseSessionId, studentUserId, hours]);
        reservationId = Number(insertResult.insertId);
    }
    for (const source of sources) {
        if (source.sourceType === 'grant') {
            await conn.query(`UPDATE platform_lesson_hour_grants
         SET remaining_hours = remaining_hours - ?
         WHERE id = ? AND remaining_hours >= ?`, [source.hours, source.sourceId, source.hours]);
        }
        else {
        ...
        await conn.query(`INSERT INTO course_session_hour_reservation_sources
         (reservation_id, source_type, source_id, hours)
       VALUES (?, ?, ?, ?)`, [reservationId, source.sourceType, source.sourceId, source.hours]);
    }
    await conn.query(`UPDATE users
     SET lesson_balance_hours = lesson_balance_hours - ?
     WHERE id = ? AND lesson_balance_hours >= ?`, [hours, studentUserId, hours]);
    return hours;
}
async function settleLessonHours(conn, studentUserId, courseSessionId, rawHours) {
    const hours = toHours(rawHours);
    if (hours <= 0) {
        throw new walletHours_1.WalletHoursError('HOUR_ALLOCATION_MISMATCH', '无效课时');
    }
    const [reservationRows] = await conn.query(`SELECT id, reserved_hours, status
     FROM course_session_hour_reservations
     WHERE course_session_id = ? AND student_user_id = ?
     LIMIT 1
     FOR UPDATE`, [courseSessionId, studentUserId]);
    const reservation = reservationRows?.[0];
    if (!reservation || String(reservation.status) !== 'active') {
        return (0, walletHours_1.consumeLessonHours)(conn, studentUserId, courseSessionId, hours);
    }
    ...
    for (const source of sourceRows || []) {
        ...
        if (sourceType === 'order' && settledFromSource > EPSILON) {
         ...
         VALUES (?, ?, ?, 'lesson', ?, ?)`, [studentUserId, sourceId, courseSessionId, `lesson:${courseSessionId}:reserved`, settledFromSource]);
        }
        await restoreSourceHours(conn, sourceType, sourceId, releasedFromSource);
    }
    if (releasedHours > EPSILON) {
        await conn.query('UPDATE users SET lesson_balance_hours = lesson_balance_hours + ? WHERE id = ?', [releasedHours, studentUserId]);
    }
    const extraHours = Number((hours - reservedHours).toFixed(2));
    if (extraHours > EPSILON) {
        await (0, walletHours_1.consumeLessonHours)(conn, studentUserId, courseSessionId, extraHours);
    }
    await conn.query(`UPDATE course_session_hour_reservations
     SET status = 'settled', settled_hours = ?, settled_at = CURRENT_TIMESTAMP
     WHERE id = ?`, [hours, reservationId]);
    return [];
}
