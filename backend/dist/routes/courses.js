...
const isMissingCourseAlertColumnError = (e) => {
    const code = String(e?.code || '');
    const message = String(e?.message || '');
    if (!(code === 'ER_BAD_FIELD_ERROR' || message.includes('Unknown column')))
        return false;
...
const ensureCourseReviewSchema = async () => {
  ...
  `);
    ...
    const pairIndexReady = await ensureIndex('ALTER TABLE course_session_reviews ADD KEY idx_course_reviews_student_mentor (student_user_id, mentor_user_id)');
...
const ensureCourseReviewRewardSchema = async () => {
    if (courseReviewRewardSchemaEnsured)
        return true;
    await (0, db_1.query)(`
  ...
  `);
    const grantColumns = await (0, db_1.query)("SHOW COLUMNS FROM platform_lesson_hour_grants LIKE 'dispute_id'");
    const disputeIdNullable = String(grantColumns?.[0]?.Null || grantColumns?.[0]?.null || '').toUpperCase() === 'YES';
    if (!disputeIdNullable) {
        await (0, db_1.query)('ALTER TABLE platform_lesson_hour_grants MODIFY COLUMN dispute_id BIGINT NULL');
    }
    await (0, db_1.query)(`
...
const ensureCourseAlertColumns = async () => {
    ...
    const studentReady = await ensureColumn('ALTER TABLE account_settings ADD COLUMN student_last_seen_course_id BIGINT UNSIGNED NOT NULL DEFAULT 0');
    const mentorReady = await ensureColumn('ALTER TABLE account_settings ADD COLUMN mentor_last_seen_course_id BIGINT UNSIGNED NOT NULL DEFAULT 0');
...
const fetchLastSeenCourseId = async (userId, view) => {
    ...
    try {
        const rows = await (0, db_1.query)(selectSql, [userId]);
        return Math.max(0, toInt(rows?.[0]?.last_seen_course_id) ?? 0);
    }
    catch (e) {
        if (!isMissingCourseAlertColumnError(e))
            return 0;
...
router.post('/alerts/seen', auth_1.requireAuth, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ error: 'unauthorized' });
  ...
  `;
    try {
        try {
            await (0, db_1.query)(upsertSql, [req.user.id, lastSeenCourseId]);
        }
        catch (e) {
            if (!isMissingCourseAlertColumnError(e))
                throw e;
...
router.get('/:courseId/replay-files', auth_1.requireAuth, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ error: 'unauthorized' });
    ...
    try {
        ...
        const recordingRows = await (0, db_1.query)(`
      ...
      `, [courseId]);
        ...
        if ((0, classroomAccess_1.isMissingClassroomSchemaError)(e) || isMissingCoursesSchemaError(e)) {
            return res.status(500).json({ error: 'courses_schema_missing' });
        }
    ...
    try {
      ...
      `, [courseId, req.user.id]);
        ...
        `, [req.user.id, mentorUserId]);
            hadPriorMentorReview = Boolean(priorReviewRowsRaw?.[0]);
        }
        ...
        if (!existing && !hadPriorMentorReview && reviewId > 0) {
          ...
          `, [req.user.id, mentorUserId, courseId, reviewId, COURSE_REVIEW_REWARD_HOURS]);
                ...
                if (String(claimError?.code || '') !== 'ER_DUP_ENTRY')
                    throw claimError;
            }
            if (rewardClaimId > 0) {
                const grantPublicId = `review-${crypto_1.default.randomBytes(12).toString('hex')}`;
          ...
          `, [grantPublicId, req.user.id, COURSE_REVIEW_REWARD_HOURS, COURSE_REVIEW_REWARD_HOURS]);
                ...
                await connection.query('UPDATE course_review_reward_grants SET platform_grant_id = ? WHERE id = ?', [platformGrantId, rewardClaimId]);
                await connection.query('UPDATE users SET lesson_balance_hours = lesson_balance_hours + ? WHERE id = ?', [COURSE_REVIEW_REWARD_HOURS, req.user.id]);
                const [balanceRowsRaw] = await connection.query('SELECT lesson_balance_hours FROM users WHERE id = ? LIMIT 1', [req.user.id]);
                lessonBalanceHours = toNumber(balanceRowsRaw?.[0]?.lesson_balance_hours);
