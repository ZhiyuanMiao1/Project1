...
const normalizePreciseRating = (raw, digits = 2) => {
    const n = typeof raw === 'number' ? raw : Number.parseFloat(String(raw ?? '0'));
...
const isMissingCourseReviewSchemaError = (e) => {
    const code = String(e?.code || '');
    const message = String(e?.message || '');
    return (code === 'ER_NO_SUCH_TABLE'
        || ((code === 'ER_BAD_FIELD_ERROR' || message.includes('Unknown column')) && (message.includes('course_session_reviews')
...
const normalizeMentorReviewRows = (rows = []) => {
    return rows.map((row) => {
...
router.get('/:mentorId/detail', async (req, res) => {
    ...
    try {
        ...
        if (!row)
            return res.status(404).json({ error: 'mentor_not_found' });
        const courses = parseCourses(row.courses_json);
        const mentor = {
            id: row.public_id,
        ...
        if (Number.isFinite(mentorUserId) && mentorUserId > 0) {
            ...
            `, [mentorUserId]),
                ...
                if (!isMissingCourseReviewSchemaError(e))
                    throw e;
            }
        }
