...
const parseLessonHoursConfirmationPayload = (payloadJson) => {
    const parsed = safeJsonParse(payloadJson);
    if (!parsed || typeof parsed !== 'object')
        return null;
    if (parsed.kind !== 'lesson_hours_confirmation')
        return null;
    return parsed;
};
...
const toLessonHoursConfirmationCard = (row, currentUserId) => {
...
const refreshMentorResponseTimeMetricIfNeeded = async (conn, row, actingUserId) => {
    ...
    if (actingUserId !== mentorUserId)
        return;
    await (0, mentorRecommendation_1.touchMentorLastRepliedWithConnection)(conn, mentorUserId);
...
const resolveRestoredAppointmentStatus = async (conn, threadId, appointmentId) => {
    const [decisionRows] = await conn.execute(`
    SELECT payload_json
    FROM message_items
    WHERE thread_id = ?
...
const findFallbackRescheduleSourceAppointmentId = async (conn, { threadId, recalledAppointmentId, recalledPayload, }) => {
    ...
    for (const candidateRow of candidateRows || []) {
        ...
        if (recalledDirectionId && safeText(candidatePayload?.courseDirectionId) !== recalledDirectionId)
            continue;
        if (recalledTypeId && safeText(candidatePayload?.courseTypeId) !== recalledTypeId)
            continue;
        return candidateId;
    }
    return null;
};
const restoreRescheduleSourceAfterRecall = async (conn, row, recalledAppointmentId) => {
    ...
    if (sourceAppointmentId == null || sourceAppointmentId === recalledAppointmentId)
        return;
    ...
    if (sourceStatus !== 'rescheduling')
        return;
    const restoredStatus = await resolveRestoredAppointmentStatus(conn, threadId, sourceAppointmentId);
    if (restoredStatus === 'pending') {
        await conn.execute('DELETE FROM appointment_statuses WHERE appointment_message_id = ? LIMIT 1', [sourceAppointmentId]);
        return;
    }
    await conn.execute(`
...
const normalizeCourseSessionStatus = (value) => {
    const raw = typeof value === 'string' ? value.trim().toLowerCase() : '';
    return raw;
};
const isCancelledCourseSessionStatus = (value) => {
    const status = normalizeCourseSessionStatus(value);
    return status === 'cancelled' || status === 'canceled';
};
const markCourseRequestPairedForAcceptedAppointment = async (conn, payload, studentUserId) => {
    const courseRequestId = toPositiveIntOrNull(payload?.courseRequestId);
    if (courseRequestId == null)
        return;
    if (!Number.isFinite(studentUserId) || studentUserId <= 0)
        return;
    await conn.execute(`
...
const syncCourseSessionForAppointmentDecision = async (conn, row, status) => {
    ...
    const startsAt = formatUtcDatetime(parsed.startsAtUtc);
    const sessionStatus = parsed.endsAtUtc.getTime() <= Date.now() ? 'completed' : 'scheduled';
    const [existingRows] = await conn.execute('SELECT id, status FROM course_sessions WHERE student_user_id = ? AND mentor_user_id = ? AND starts_at = ? LIMIT 1', [studentUserId, mentorUserId, startsAt]);
    ...
    if (existingStatus === 'completed' || isCancelledCourseSessionStatus(existingStatus))
        return;
    await conn.execute(`
    UPDATE course_sessions
...
const getUserRoleInThread = (userId, studentUserId, mentorUserId) => {
    if (Number.isFinite(userId) && userId === studentUserId)
        return 'student';
    if (Number.isFinite(userId) && userId === mentorUserId)
        return 'mentor';
    return '';
};
...
const getAppointmentActorDisplayName = async (actorUserId, studentUserId, mentorUserId) => {
    ...
    const rows = role === 'mentor'
        ...
        : role
          ...
          LIMIT 1
          `, [role, actorUserId])
            : await (0, db_1.query)('SELECT username, NULL AS public_id, NULL AS display_name FROM users WHERE id = ? LIMIT 1', [actorUserId]);
    const row = rows?.[0] || {};
...
router.post('/appointments', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (shouldSendAsMentor) {
            ...
            const mentorRoleRows = await (0, db_1.query)("SELECT public_id FROM user_roles WHERE user_id = ? AND role = 'mentor' LIMIT 1", [req.user.id]);
        ...
        `, [Number.isFinite(messageId) && messageId > 0 ? messageId : null, threadId]);
            void (0, mentorRecommendation_1.touchMentorLastReplied)(req.user.id).catch((error) => {
...
router.post('/threads/:threadId/appointments', auth_1.requireAuth, async (req, res) => {
    ...
    try {
      ...
      `, [threadId, req.user.id, req.user.id]);
        ...
        if (sourceAppointmentId != null) {
            const sourceRows = await (0, db_1.query)(`
        SELECT
          mi.id,
          mi.thread_id,
          mi.sender_user_id,
        ...
        `, [sourceAppointmentId, threadId]);
            const sourceRow = sourceRows?.[0];
            if (!sourceRow)
                return res.status(404).json({ error: '来源预约不存在或无权限' });
            const sourceStatus = normalizeDecisionStatus(sourceRow.appointment_status) || 'pending';
            ...
            if (!sourcePayload)
                return res.status(400).json({ error: '来源预约数据无效' });
            ...
            if (!nextWindow)
                return res.status(400).json({ error: '预约时间格式无效' });
            if (appointmentIntent !== 'reschedule'
                && sourceWindow
      ...
      `, [messageId, threadId]);
        ...
        if (req.user.id === mentorUserId) {
            void (0, mentorRecommendation_1.touchMentorLastReplied)(mentorUserId).catch((error) => {
                console.error('Touch mentor last replied error:', error);
            });
        }
...
router.post('/lesson-hour-confirmations/:messageId/respond', auth_1.requireAuth, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ error: '未授权' });
...
router.post('/lesson-hour-confirmations/:messageId/retry', auth_1.requireAuth, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ error: '未授权' });
    const messageId = Number(req.params.messageId);
    if (!Number.isFinite(messageId) || messageId <= 0) {
...
router.post('/threads/:threadId/star', auth_1.requireAuth, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ error: '未授权' });
    const threadId = Number(req.params.threadId);
    if (!Number.isFinite(threadId) || threadId <= 0) {
...
router.delete('/threads/:threadId/star', auth_1.requireAuth, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ error: '未授权' });
    const threadId = Number(req.params.threadId);
    if (!Number.isFinite(threadId) || threadId <= 0) {
...
router.post('/threads/:threadId/archive', auth_1.requireAuth, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ error: '未授权' });
    const threadId = Number(req.params.threadId);
    if (!Number.isFinite(threadId) || threadId <= 0) {
    ...
    try {
      ...
      `, [req.user.id, req.user.id, req.user.id, req.user.id, req.user.id]);
        const items = (rows || [])
            ...
            if ((actionRole === 'student' && status !== 'pending')
      ...
      `, [req.user.id, req.user.id, req.user.id, req.user.id]);
        ...
        const threads = (rows || []).map((r) => {
            ...
            return {
                ...
                latestLessonHoursConfirmation: null,
                messages: [],
                unreadCount: 0,
            };
        });
        ...
        `, [req.user.id, req.user.id, req.user.id, ...threadIds]);
            ...
            const lessonHourRows = await (0, db_1.query)(`
        SELECT
          mi.id,
          mi.thread_id,
          mi.sender_user_id,
        ...
        ORDER BY mi.thread_id ASC, mi.id ASC
        `, [req.user.id, req.user.id, req.user.id, ...threadIds]);
            const lessonHourByThread = new Map();
            for (const row of lessonHourRows || []) {
                const tid = String(row?.thread_id || '').trim();
                if (!tid)
                    continue;
                if (!lessonHourByThread.has(tid))
                    lessonHourByThread.set(tid, []);
          ...
          `, pairParams);
            ...
            for (const t of threads) {
                ...
                const lessonHourConfirmations = recentRows
                    ...
                    if (cardId != null && hiddenAppointmentIds.has(cardId))
                        return null;
                    return card;
                })
                    .filter(Boolean);
