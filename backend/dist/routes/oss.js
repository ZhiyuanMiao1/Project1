...
router.post('/policy', [
    ...
    try {
        ...
        if (scope === 'mentorAvatar') {
            ...
            const roleRows = await (0, db_1.query)("SELECT public_id FROM user_roles WHERE user_id = ? AND role = 'mentor' LIMIT 1", [req.user.id]);
            const row = roleRows?.[0];
            if (!row)
                return res.status(403).json({ error: '未开通导师身份' });
