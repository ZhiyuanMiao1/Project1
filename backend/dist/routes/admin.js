...
router.post('/course-disputes/:disputeId/accept', adminAuth_1.requireAdminAuth, async (req, res) => {
    const disputeId = safeString(req.params.disputeId, 40);
    const version = toPositiveInt(req.body?.version, 0);
    try {
        const result = await (0, db_1.query)(`UPDATE course_session_disputes SET status = 'reviewing', assigned_admin_id = ?, accepted_at = COALESCE(accepted_at, CURRENT_TIMESTAMP), version = version + 1
       WHERE public_id = ? AND status IN ('submitted','reviewing') AND version = ?`, [req.admin?.adminId, disputeId, version]);
        if (Number(result?.affectedRows || 0) !== 1)
            return res.status(409).json({ error: '异议已被其他管理员更新，请刷新' });
        const rows = await (0, db_1.query)('SELECT * FROM course_session_disputes WHERE public_id = ? LIMIT 1', [disputeId]);
        await (0, db_1.query)(`INSERT INTO course_dispute_events (dispute_id, admin_id, event_type) VALUES (?, ?, 'accepted')`, [rows[0].id, req.admin?.adminId]);
...
router.post('/course-disputes/:disputeId/resolve', adminAuth_1.requireAdminAuth, async (req, res) => {
    ...
    try {
        ...
        if (Number(dispute.version || 1) !== version || !['submitted', 'reviewing'].includes(String(dispute.status))) {
            await conn.rollback();
    ...
    if (refundIds.length) {
        ...
        try {
            ...
            await (0, db_1.query)(`UPDATE course_session_disputes SET status = 'action_pending', refund_status = 'failed', version = version + 1 WHERE public_id = ?`, [publicId]);
        ...
        if (processed.length) {
            await (0, db_1.query)(`UPDATE course_session_disputes SET status = ?, refund_status = ?, resolved_at = ?, version = version + 1 WHERE public_id = ?`, [allCompleted ? 'resolved' : 'action_pending', allCompleted ? 'completed' : anyFailed ? 'failed' : 'processing', allCompleted ? new Date() : null, publicId]);
            finalRow = (await (0, db_1.query)('SELECT csd.*, u.email AS student_email FROM course_session_disputes csd JOIN users u ON u.id = csd.student_user_id WHERE csd.public_id = ? LIMIT 1', [publicId]))[0];
        }
    }
...
router.post('/course-disputes/:disputeId/refresh-refunds', adminAuth_1.requireAdminAuth, async (req, res) => {
    ...
    if (!dispute)
        return res.status(404).json({ error: '未找到异议' });
    if (String(dispute.status) !== 'action_pending' || String(dispute.outcome_code) !== 'refund')
    ...
    const anyFailed = processed.some((row) => String(row?.status || '').toUpperCase() === 'FAILED');
    await (0, db_1.query)(`UPDATE course_session_disputes SET status = ?, refund_status = ?, resolved_at = CASE WHEN ? THEN CURRENT_TIMESTAMP ELSE resolved_at END, version = version + 1 WHERE id = ?`, [allCompleted ? 'resolved' : 'action_pending', allCompleted ? 'completed' : anyFailed ? 'failed' : 'processing', allCompleted ? 1 : 0, dispute.id]);
