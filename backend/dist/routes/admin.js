...
const mentorPayroll_1 = require("../services/mentorPayroll");
...
const parsePayrollMonth = (value) => {
    const month = safeString(value, 7);
    if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(month))
        return null;
    return month;
};
const getPayrollMonthRange = (month) => {
...
const loadDisputeRefundQuote = async (conn, dispute, requestedHours) => {
    const [allocationRows] = await conn.query(`SELECT bha.billing_order_id, SUM(bha.hours) AS allocated_hours,
     ...
     FOR UPDATE`, [dispute.course_session_id, dispute.student_user_id]);
    const [handledRows] = await conn.query(`SELECT COALESCE(SUM(cdr.hours), 0) AS handled_hours
     FROM course_dispute_refunds cdr WHERE cdr.dispute_id = ?`, [dispute.id]);
    const alreadyHandled = Number(handledRows?.[0]?.handled_hours || 0);
    const maxHours = Number(Math.min(Number(dispute.final_hours || dispute.duration_hours || 0), Math.max(0, (allocationRows || []).reduce((sum, row) => sum + Number(row.allocated_hours || 0), 0) - alreadyHandled)).toFixed(2));
    if (requestedHours > maxHours + 0.000001)
        throw Object.assign(new Error('退款课时超过本节可处理上限'), { statusCode: 409 });
    ...
    for (const order of allocationRows || []) {
        ...
        if (lineHours <= 0)
            continue;
        const [refundRows] = await conn.query(`SELECT COALESCE(SUM(requested_hours),0) AS hours,
              COALESCE(SUM(amount_cny),0) AS cny,
              COALESCE(SUM(amount_original),0) AS original
       FROM billing_refunds
       WHERE billing_order_id = ? AND status IN ('PENDING','PROCESSING','COMPLETED')`, [order.billing_order_id]);
        const prior = refundRows?.[0] || {};
        const quote = (0, refundPricing_1.computeRefundPricing)({
        ...
        if (quote.refundAmountOriginal < 0.01)
            throw Object.assign(new Error('优惠重算后该课时暂无可退金额'), { statusCode: 422 });
        lines.push({
...
router.get('/dashboard/summary', adminAuth_1.requireAdminAuth, async (req, res) => {
    ...
    try {
       ...
       FROM users WHERE id = ? LIMIT 1`, [userId]);
        ...
        const [roles, mentorProfiles, orderSummary, courseSummary, reviews] = await Promise.all([
         ...
         ORDER BY csr.created_at DESC, csr.id DESC`, [userId]),
        ]);
...
router.delete('/reviews/:reviewId/comment', adminAuth_1.requireAdminAuth, async (req, res) => {
    const reviewId = toPositiveInt(req.params.reviewId, 0);
    const reason = readReason(req);
    if (!reviewId)
        return res.status(400).json({ error: '无效评价ID' });
    if (!reason)
        return res.status(400).json({ error: '请填写删除原因' });
    try {
        const beforeRows = await (0, db_1.query)(`SELECT id, course_session_id, student_user_id, mentor_user_id, comment_text, created_at, updated_at
       FROM course_session_reviews
       WHERE id = ?
       LIMIT 1`, [reviewId]);
        const before = beforeRows?.[0];
        if (!before)
            return res.status(404).json({ error: '未找到评价' });
        if (!safeString(before.comment_text, 10000)) {
            return res.status(409).json({ error: '该评价已无文字评论' });
        }
        await (0, db_1.query)(`UPDATE course_session_reviews
...
router.get('/mentors/:userId/contract-preview', async (req, res) => {
    const userId = toPositiveInt(req.params.userId, 0);
    if (!userId)
        return res.status(400).json({ error: '无效导师ID' });
    try {
        ...
        if (!admin)
            return res.status(401).json({ error: '后台登录已失效' });
        const rows = await (0, db_1.query)(`SELECT mcs.id, mcs.contract_number, mcs.template_file_name, mcs.final_pdf_oss_key
       FROM user_roles ur
       ...
       LIMIT 1`, [userId]);
        ...
        if (!contract?.final_pdf_oss_key)
            return res.status(404).json({ error: '未找到已签署合同' });
        const client = (0, ossClient_1.getOssClient)();
        if (!client)
            return res.status(500).json({ error: 'OSS 未配置' });
...
router.post('/orders/:orderId/complete-manual-refund', adminAuth_1.requireAdminAuth, async (req, res) => {
    const orderId = toPositiveInt(req.params.orderId, 0);
    if (!orderId)
        return res.status(400).json({ error: '无效订单ID' });
    let conn = null;
    ...
    let after = [];
    let provider = '';
    let completedDisputes = [];
    try {
        conn = await db_1.pool.getConnection();
        await conn.beginTransaction();
        const [orderRows] = await conn.query(`SELECT id, provider, credited_at
       FROM billing_orders
       WHERE id = ?
       LIMIT 1
       FOR UPDATE`, [orderId]);
        ...
        const [refundRows] = await conn.query(`SELECT id, public_id, requested_hours, amount_cny, status, balance_reserved,
       ...
       FOR UPDATE`, [orderId]);
        ...
        if (!pendingRefunds.length) {
            const alreadyCompleted = before.some((refund) => String(refund.status || '').toUpperCase() === 'COMPLETED');
            if (alreadyCompleted) {
                await conn.commit();
       ...
       WHERE billing_order_id = ?
         ...
         AND status IN ('PENDING', 'PROCESSING')`, [orderId]);
        const [updatedRows] = await conn.query(`SELECT id, public_id, requested_hours, amount_cny, status, balance_reserved,
              created_at, completed_at
       FROM billing_refunds
       WHERE billing_order_id = ?
       ORDER BY id ASC`, [orderId]);
        after = updatedRows || [];
        const [candidateDisputes] = await conn.query(`SELECT DISTINCT csd.id, csd.public_id, csd.student_user_id, csd.outcome_code, csd.result_message, u.email AS student_email
       FROM course_dispute_refunds cdr
       JOIN course_session_disputes csd ON csd.id = cdr.dispute_id
       JOIN users u ON u.id = csd.student_user_id
       WHERE cdr.billing_refund_id IN (${pendingRefunds.map(() => '?').join(',')})`, pendingRefunds.map((refund) => refund.id));
        for (const dispute of candidateDisputes || []) {
            const [openRows] = await conn.query(`SELECT COUNT(*) AS count FROM course_dispute_refunds cdr JOIN billing_refunds br ON br.id = cdr.billing_refund_id WHERE cdr.dispute_id = ? AND br.status <> 'COMPLETED'`, [dispute.id]);
        ...
        return res.json({
            ...
            completedAmountCny: Number(pendingRefunds.reduce((sum, refund) => sum + Number(refund.amount_cny || 0), 0).toFixed(2)),
        });
    }
    catch (error) {
        try {
            ...
            streams: [
                {
                    ...
                    authInfo: (0, aliyunRtc_1.toAliRtcSdkAuthInfo)(studentAuth),
                ...
                },
                {
                    ...
                    authInfo: (0, aliyunRtc_1.toAliRtcSdkAuthInfo)(mentorAuth),
...
router.get('/mentor-payroll', adminAuth_1.requireAdminAuth, async (req, res) => {
    ...
    try {
       ...
       ORDER BY COALESCE(pay.gross_income_cny, earned.settled_hours * COALESCE(mpp.hourly_rate_cny, ?)) DESC,
        ...
        const payroll = (rows || []).map((row) => {
            ...
            const calculated = (0, mentorPayroll_1.calculateMentorPayroll)(grossIncomeCny, chinaTaxResident);
            return {
...
router.put('/mentor-payroll/:mentorUserId/profile', adminAuth_1.requireAdminAuth, async (req, res) => {
    const mentorUserId = toPositiveInt(req.params.mentorUserId, 0);
    const hourlyRateCny = Number(req.body?.hourlyRateCny);
    const chinaTaxResident = req.body?.chinaTaxResident;
    if (!mentorUserId)
    ...
    if (typeof chinaTaxResident !== 'boolean')
        return res.status(400).json({ error: '请选择是否在中国纳税' });
    try {
        ...
        const beforeRows = await (0, db_1.query)('SELECT * FROM mentor_payroll_profiles WHERE mentor_user_id = ? LIMIT 1', [mentorUserId]);
        await (0, db_1.query)(`INSERT INTO mentor_payroll_profiles (mentor_user_id, hourly_rate_cny, china_tax_resident, updated_by_admin_id)
       VALUES (?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE hourly_rate_cny = VALUES(hourly_rate_cny),
         china_tax_resident = VALUES(china_tax_resident), updated_by_admin_id = VALUES(updated_by_admin_id)`, [mentorUserId, Number(hourlyRateCny.toFixed(2)), chinaTaxResident ? 1 : 0, req.admin?.adminId || null]);
...
router.post('/mentor-payroll/:mentorUserId/pay', adminAuth_1.requireAdminAuth, async (req, res) => {
    const mentorUserId = toPositiveInt(req.params.mentorUserId, 0);
    const month = parsePayrollMonth(req.body?.month);
    const paymentReference = safeString(req.body?.paymentReference, 120);
    const note = safeString(req.body?.note, 1000);
    if (!mentorUserId || !month)
        return res.status(400).json({ error: '导师或月份无效' });
    const { start, end } = getPayrollMonthRange(month);
    const defaultHourlyRate = getDefaultMentorHourlyRate();
    const conn = await db_1.pool.getConnection();
    try {
        await conn.beginTransaction();
        const [existingRows] = await conn.query('SELECT id FROM mentor_payroll_payments WHERE mentor_user_id = ? AND payroll_month = ? FOR UPDATE', [mentorUserId, month]);
        if (existingRows?.length)
       ...
       WHERE lhc.mentor_user_id = ? AND lhc.final_hours IS NOT NULL
        ...
        const grossIncomeCny = Number((settledHours * hourlyRateCny).toFixed(2));
        if (grossIncomeCny <= 0)
            throw Object.assign(new Error('该导师本月暂无可发放收入'), { statusCode: 422 });
        const chinaTaxResident = Boolean(Number(profile.china_tax_resident));
        const tax = (0, mentorPayroll_1.calculateMentorPayroll)(grossIncomeCny, chinaTaxResident);
        const [insertResult] = await conn.query(`INSERT INTO mentor_payroll_payments
        (mentor_user_id, payroll_month, settled_hours, hourly_rate_cny, gross_income_cny,
         china_tax_resident, taxable_income_cny, withheld_tax_cny, net_income_cny,
         payment_reference, note_text, paid_by_admin_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [mentorUserId, month, settledHours, hourlyRateCny, grossIncomeCny, chinaTaxResident ? 1 : 0,
        ...
        await conn.query(`INSERT INTO admin_audit_logs
        (admin_id, action, target_type, target_id, reason, before_json, after_json, ip, user_agent)
       VALUES (?, 'mentor_payroll.payment.mark_paid', 'mentor_payroll', ?, ?, NULL, ?, ?, ?)`, [req.admin?.adminId || null, `${mentorUserId}:${month}`, note || null, jsonOrNull(after),
            safeString(req.ip || '', 45) || null, safeString(req.get('user-agent') || '', 255) || null]);
        await conn.commit();
...
router.get('/course-disputes/stats', adminAuth_1.requireAdminAuth, async (_req, res) => {
    try {
        const rows = await (0, db_1.query)(`SELECT status, COUNT(*) AS count FROM course_session_disputes GROUP BY status`);
...
router.get('/course-disputes', adminAuth_1.requireAdminAuth, async (req, res) => {
    const { page, limit, offset } = getPaging(req);
    const q = safeString(req.query.q, 100);
    ...
    if (COURSE_DISPUTE_STATUSES.has(status)) {
        where.push('csd.status = ?');
        params.push(status);
    }
    if (reason) {
        where.push('csd.reason_code = ?');
        params.push(reason);
    }
    if (resolution) {
        where.push('csd.preferred_resolution = ?');
        params.push(resolution);
    }
    if (startDate) {
        where.push('csd.created_at >= ?');
        params.push(`${startDate} 00:00:00`);
    }
    if (endDate) {
        where.push('csd.created_at <= ?');
    ...
    try {
        const countRows = await (0, db_1.query)(`SELECT COUNT(*) AS total ${joins} WHERE ${where.join(' AND ')}`, params);
        const rows = await (0, db_1.query)(`SELECT csd.*, sr.public_id AS student_public_id, mr.public_id AS mentor_public_id,
       ...
       ${joins} WHERE ${where.join(' AND ')} ORDER BY csd.created_at DESC ${pagingSql(limit, offset)}`, params);
...
router.get('/course-disputes/:disputeId', adminAuth_1.requireAdminAuth, async (req, res) => {
    const disputeId = safeString(req.params.disputeId, 40);
    try {
        const rows = await (0, db_1.query)(`SELECT csd.*, cs.course_direction, cs.course_type, cs.starts_at, cs.duration_hours,
       ...
       WHERE csd.public_id = ? LIMIT 1`, [disputeId]);
        const dispute = rows?.[0];
        if (!dispute)
            return res.status(404).json({ error: '未找到异议' });
        const [events, refunds, allocations] = await Promise.all([
            (0, db_1.query)(`SELECT cde.*, au.display_name, au.username FROM course_dispute_events cde LEFT JOIN admin_users au ON au.id = cde.admin_id WHERE cde.dispute_id = ? ORDER BY cde.id ASC`, [dispute.id]),
            (0, db_1.query)(`SELECT br.*, cdr.hours AS dispute_hours FROM course_dispute_refunds cdr JOIN billing_refunds br ON br.id = cdr.billing_refund_id WHERE cdr.dispute_id = ? ORDER BY cdr.id ASC`, [dispute.id]),
            (0, db_1.query)(`SELECT bha.billing_order_id, SUM(bha.hours) AS hours, bo.provider, bo.currency_code, bo.amount_cny FROM billing_hour_allocations bha JOIN billing_orders bo ON bo.id = bha.billing_order_id WHERE bha.course_session_id = ? GROUP BY bha.billing_order_id, bo.provider, bo.currency_code, bo.amount_cny`, [dispute.course_session_id]),
        ]);
...
router.post('/course-disputes/:disputeId/notes', adminAuth_1.requireAdminAuth, async (req, res) => {
    const disputeId = safeString(req.params.disputeId, 40);
    const note = safeString(req.body?.note, 4000);
    if (!note)
        return res.status(400).json({ error: '请输入内部备注' });
    const rows = await (0, db_1.query)('SELECT id FROM course_session_disputes WHERE public_id = ? LIMIT 1', [disputeId]);
    if (!rows?.[0])
        return res.status(404).json({ error: '未找到异议' });
    await (0, db_1.query)(`INSERT INTO course_dispute_events (dispute_id, admin_id, event_type, note_text) VALUES (?, ?, 'internal_note', ?)`, [rows[0].id, req.admin?.adminId, note]);
    await audit({ req, action: 'course_dispute.note', targetType: 'course_dispute', targetId: disputeId, reason: note });
    return res.status(201).json({ ok: true });
});
router.post('/course-disputes/:disputeId/refund-quote', adminAuth_1.requireAdminAuth, async (req, res) => {
    const hours = (0, refundPricing_1.parseRefundHours)(req.body?.hours);
    if (!hours)
        return res.status(400).json({ error: '退款课时需为0.25小时倍数' });
    const conn = await db_1.pool.getConnection();
    try {
        await conn.beginTransaction();
        const [rows] = await conn.query(`SELECT csd.*, cs.duration_hours, lhc.final_hours FROM course_session_disputes csd JOIN course_sessions cs ON cs.id = csd.course_session_id LEFT JOIN lesson_hour_confirmations lhc ON lhc.id = (SELECT MAX(x.id) FROM lesson_hour_confirmations x WHERE x.course_session_id = cs.id) WHERE csd.public_id = ? LIMIT 1 FOR UPDATE`, [safeString(req.params.disputeId, 40)]);
        if (!rows?.[0]) {
            await conn.rollback();
            return res.status(404).json({ error: '未找到异议' });
        }
...
router.post('/course-disputes/:disputeId/resolve', adminAuth_1.requireAdminAuth, async (req, res) => {
    const publicId = safeString(req.params.disputeId, 40);
    const outcome = safeString(req.body?.outcome, 32);
    const resultMessage = safeString(req.body?.resultMessage, 4000);
    const version = toPositiveInt(req.body?.version, 0);
    const hours = (0, refundPricing_1.parseRefundHours)(req.body?.hours);
    if (!COURSE_DISPUTE_OUTCOMES.has(outcome))
        return res.status(400).json({ error: '无效处理结果' });
    if (!resultMessage)
        return res.status(400).json({ error: '请填写学生可见的处理说明' });
    if ((outcome === 'lesson_credit' || outcome === 'refund') && !hours)
        return res.status(400).json({ error: '课时需为0.25小时倍数' });
    const conn = await db_1.pool.getConnection();
    let finalRow = null;
    let refundIds = [];
    try {
        await conn.beginTransaction();
        const [rows] = await conn.query(`SELECT csd.*, cs.duration_hours, su.email AS student_email, lhc.final_hours FROM course_session_disputes csd JOIN course_sessions cs ON cs.id = csd.course_session_id JOIN users su ON su.id = csd.student_user_id LEFT JOIN lesson_hour_confirmations lhc ON lhc.id = (SELECT MAX(x.id) FROM lesson_hour_confirmations x WHERE x.course_session_id = cs.id) WHERE csd.public_id = ? LIMIT 1 FOR UPDATE`, [publicId]);
        const dispute = rows?.[0];
        if (!dispute) {
            await conn.rollback();
            return res.status(404).json({ error: '未找到异议' });
        }
        if (Number(dispute.version || 1) !== version || String(dispute.status) !== 'submitted') {
            await conn.rollback();
            return res.status(409).json({ error: '异议已被更新，请刷新' });
        }
        ...
        if (hours && hours > maxHours + 0.000001) {
            await conn.rollback();
            return res.status(409).json({ error: '处理课时超过本节实际扣除' });
        }
        let nextStatus = outcome === 'rejected' ? 'rejected' : 'resolved';
        let refundStatus = null;
        if (outcome === 'lesson_credit') {
            const grantId = `PC${Date.now().toString(36)}${crypto_1.default.randomBytes(4).toString('hex')}`.toUpperCase();
        ...
        await conn.commit();
        const afterRows = await (0, db_1.query)('SELECT csd.*, u.email AS student_email FROM course_session_disputes csd JOIN users u ON u.id = csd.student_user_id WHERE csd.id = ? LIMIT 1', [dispute.id]);
    ...
    if (refundIds.length) {
        let processed = [];
        try {
            processed = await Promise.all(refundIds.map((id) => (0, refunds_1.processRefundById)(id)));
        }
        catch (error) {
            console.error('Course dispute refund execution error:', error);
            await (0, db_1.query)(`UPDATE course_session_disputes SET status = 'submitted', refund_status = 'failed', version = version + 1 WHERE public_id = ?`, [publicId]);
            await (0, db_1.query)(`INSERT INTO course_dispute_events (dispute_id, admin_id, event_type, note_text, payload_json)
         SELECT id, ?, 'refund_failed', ?, ? FROM course_session_disputes WHERE public_id = ?`, [req.admin?.adminId, error?.message || '退款渠道执行失败', JSON.stringify({ refundIds }), publicId]);
            finalRow = (await (0, db_1.query)('SELECT csd.*, u.email AS student_email FROM course_session_disputes csd JOIN users u ON u.id = csd.student_user_id WHERE csd.public_id = ? LIMIT 1', [publicId]))[0];
        }
        const allCompleted = processed.every((row) => String(row?.status || '').toUpperCase() === 'COMPLETED');
        const anyFailed = processed.some((row) => String(row?.status || '').toUpperCase() === 'FAILED');
        if (processed.length) {
            await (0, db_1.query)(`UPDATE course_session_disputes SET status = ?, refund_status = ?, resolved_at = ?, version = version + 1 WHERE public_id = ?`, [allCompleted ? 'resolved' : 'submitted', allCompleted ? 'completed' : anyFailed ? 'failed' : 'processing', allCompleted ? new Date() : null, publicId]);
            finalRow = (await (0, db_1.query)('SELECT csd.*, u.email AS student_email FROM course_session_disputes csd JOIN users u ON u.id = csd.student_user_id WHERE csd.public_id = ? LIMIT 1', [publicId]))[0];
        }
    }
...
router.post('/course-disputes/:disputeId/refresh-refunds', adminAuth_1.requireAdminAuth, async (req, res) => {
    const publicId = safeString(req.params.disputeId, 40);
    const rows = await (0, db_1.query)(`SELECT csd.*, u.email AS student_email FROM course_session_disputes csd JOIN users u ON u.id = csd.student_user_id WHERE csd.public_id = ? LIMIT 1`, [publicId]);
    const dispute = rows?.[0];
    if (!dispute)
        return res.status(404).json({ error: '未找到异议' });
    if (String(dispute.status) !== 'submitted' || String(dispute.outcome_code) !== 'refund')
        return res.status(409).json({ error: '该异议没有待执行退款' });
    const refundRows = await (0, db_1.query)(`SELECT br.id, br.status, br.provider FROM course_dispute_refunds cdr JOIN billing_refunds br ON br.id = cdr.billing_refund_id WHERE cdr.dispute_id = ?`, [dispute.id]);
    for (const refund of refundRows || []) {
        if (String(refund.status).toUpperCase() === 'FAILED' && String(refund.provider).toLowerCase() === 'paypal') {
            await (0, db_1.query)(`UPDATE billing_refunds SET status = 'PROCESSING', failure_code = NULL, failure_message = NULL WHERE id = ?`, [refund.id]);
        }
    }
    let processed = [];
    try {
        processed = await Promise.all((refundRows || []).map((refund) => (0, refunds_1.processRefundById)(Number(refund.id))));
    }
    catch (error) {
        console.error('Admin refresh dispute refunds error:', error);
        await (0, db_1.query)(`UPDATE course_session_disputes SET refund_status = 'failed', version = version + 1 WHERE id = ?`, [dispute.id]);
        await (0, db_1.query)(`INSERT INTO course_dispute_events (dispute_id, admin_id, event_type, note_text) VALUES (?, ?, 'refund_failed', ?)`, [dispute.id, req.admin?.adminId, error?.message || '退款渠道执行失败']);
    ...
    const allCompleted = processed.length > 0 && processed.every((row) => String(row?.status || '').toUpperCase() === 'COMPLETED');
    const anyFailed = processed.some((row) => String(row?.status || '').toUpperCase() === 'FAILED');
